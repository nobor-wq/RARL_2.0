...
# 验证使用环境v1版本，但是模型存在v0目录下，需要对环境名做处理
env_name_for_model = args.env_name.split('-')[0] + "-v0"
base_path = os.path.join("./logs/rarl", env_name_for_model, args.algo, args.addition_msg)
if args.attack:
    advmodel_path = os.path.join(base_path, args.adv_iteration, 'adversary/best_model')
    model = PPO.load(advmodel_path, device=device)


# 加载训练好的自动驾驶模型
model_path = os.path.join(base_path, args.agent_iteration, 'agent/best_model')
...
for episode in range(args.train_step):
    ...
    for _ in range(args.T_horizon):
        ...
        if args.attack:
            ...
            actions_tensor = th.tensor(actions, device=device)  # 确保在同一设备上
            ...
            if adv_action_mask or args.unlimited_attack:
                if args.attack_method == 'fgsm':
                    adv_state = FGSM_v2(adv_actions[1], victim_agent=trained_agent,
                                        last_state=obs_tensor[:-2].unsqueeze(0), epsilon=args.attack_eps, device=device)
                elif args.attack_method == 'pgd':
                    adv_state = PGD(adv_actions[1], trained_agent, obs_tensor[:-2].unsqueeze(0),
    ...
    if env_name_for_model == 'TrafficEnv1-v0' or env_name_for_model == 'TrafficEnv3-v0' or env_name_for_model == 'TrafficEnv6-v0':
        if xa < -50.0 and ya > 4.0 and done is False:
            sn += 1
    elif env_name_for_model == 'TrafficEnv2-v0':
        if xa > 50.0 and ya > -5.0 and done is False:
            sn += 1
    elif env_name_for_model == 'TrafficEnv4-v0':
        if ya < -50.0 and done is False:
            sn += 1
    elif env_name_for_model == 'TrafficEnv7-v0':
        if done is False:
            sn += 1
    elif env_name_for_model == 'TrafficEnv8-v0':
