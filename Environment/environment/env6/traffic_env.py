"""
 Env6 is a copy of Env3, the difference is, Env6 if for ablation studies
 Three parameters are considered.
 1. without padding
 2. without clipping
 3. without victim agent's action
"""
...
class Traffic_Env(gym.Env):
    ...
    def __init__(self, attack=False, adv_steps=2, eval=False, clipping=False, v_action_obs_flag=False, remain_attack_time_flag=False):
        self.state_dim = 26
        self.action_dim = 1
        ...
        self.v_action_obs_flag = v_action_obs_flag
        self.remain_attack_time_flag = remain_attack_time_flag
        ...
        if self.attack:
            if self.v_action_obs_flag and self.remain_attack_time_flag:
                self.action_space = spaces.Box(-1.0, 1.0, shape=(2,), dtype=np.float32)
                # define dims of state space
                self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(28,), dtype=np.float32)
            elif not self.v_action_obs_flag and not self.remain_attack_time_flag:
                self.action_space = spaces.Box(-1.0, 1.0, shape=(2,), dtype=np.float32)
                # define dims of state space
    ...
    def obs_to_state(self, vehicle_params):
        ...
        if self.attack:
            if self.remain_attack_time_flag:
                state.append(self.attack_remain/self.adv_steps)
            if self.v_action_obs_flag:
                state.append(0)
        return state, info
    ...
    def step(self, action_a):
        if self.attack:
            action, mask = action_a[0].item(), action_a[1].item()
            if mask:
        ...
        if self.attack and not self.eval:
            if self.clipping and self.attack_remain == 0:
