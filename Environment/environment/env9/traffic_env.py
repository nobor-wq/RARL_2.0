        ...
        if self.AutoCarID in vehicle_params:
            ...
            for VehID in vehicle_params:
                ...
                if VehID != self.AutoCarID and dis < self.maxDistance:
                    ...
                    # calculate ACT
                    act = expert.calculate_ACT_v2(VehID, self.AutoCarID, traci.vehicle.getSpeed(self.AutoCarID), dis, angle, self.angles[-1] if self.angles else 0)
                    self.act_list.append(act)
                    self.angles.append(traci.vehicle.getAngle(self.AutoCarID))
