...
class Traffic_Env(gym.Env):
    ...
    def start(self, gui=False):
        ...
        need_rgb = (self.render_mode == "rgb_array")
        use_gui_binary = self.use_gui or need_rgb
        binary = checkBinary('sumo-gui') if use_gui_binary else checkBinary('sumo')
        ...
        if need_rgb and not self.use_gui and not is_windows:
            from pyvirtualdisplay.smartdisplay import SmartDisplay
            print("Creating a virtual display for headless rendering …")
        ...
        if LIBSUMO:
            ...
            lbl = f"lbl_{int(time.time() * 1000)}"
            traci.start(sumo_cmd, label=lbl)
            traci.getConnection(lbl)
