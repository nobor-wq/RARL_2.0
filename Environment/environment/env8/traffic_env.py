...
class Traffic_Env(gym.Env):
    ...
    def __init__(self, attack=False, adv_steps=2, eval=False, defense=False, use_gui=False, render_mode=None):
        self.state_dim = 26
        self.action_dim = 1
    ...
    def step(self, action_a):
        if self.attack and not self.defense:
            mask, action = action_a[0].item(), action_a[1].item()
            if mask:
        ...
        # print('steps is ', self.current_step, 'next state is ', next_state)
        ...
        if self.attack and not self.defense and mask and collision_value:
            info['flag'] = True
        if self.attack and not self.eval and not self.defense:
    ...
    def reset(self, seed=None, options=None):
        if seed is not None:
            self.seed(seed)  # 第一次（或有新seed时）重设随机数
        self.current_step = 0
        self.attack_remain = self.adv_steps

        sumo_seed = (self.reset_times + self._base_seed * 10000) % 1000000
        ...
        # Instead of restarting SUMO, just reload the environment
        if self.sumo_seed == "random":
            traci.load(["-c", config_path, "--random"])
        else:
            traci.load(["-c", config_path, "--seed", str(self.sumo_seed)])
        # traci.load(["-c", config_path, "--seed", str(self.sumo_seed)])
        print('Resetting the layout', self.reset_times, 'Env seed is ', self._base_seed, 'SUMO seed is', self.sumo_seed, '!!!!!!')
        self.reset_times += 1
    ...
    def get_remain_attack_times(self):
        return self.attack_remain/self.adv_steps
