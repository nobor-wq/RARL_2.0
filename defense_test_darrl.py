...
from policy import FniNet, DARRLNet
...
env = gym.make(args.env_name, attack=args.attack, adv_steps=args.adv_steps, eval=args.eval, use_gui=args.use_gui,
               render_mode=args.render_mode, darrl=True)
env.unwrapped.start()
...
if args.attack:
    ...
    elif args.algo_adv == 'DARRL':
        adv_model_path = os.path.join(args.path_adv, args.algo_adv, args.env_name, 'attacker', args.adv_algo_name)

        model = DARRLNet(state_dim=26, action_dim=1).to(device)
        model.load_state_dict(th.load(adv_model_path, map_location=device))
        model.eval()
    else:
        #advmodel_path = "./logs/adv_eval/TrafficEnv2-v0/SAC/std/lunar"
        model = PPO.load(advmodel_path, device=device)
...
for episode in range(args.train_step):
    ...
    for _ in range(args.T_horizon):
        ...
        if args.attack:
            ...
            adv_actions, _, _  = model(obs_tensor[:-2])
            if args.attack_method == 'fgsm':
                adv_state = FGSM_v2(adv_actions, victim_agent=trained_agent, last_state=obs_tensor[:-2].unsqueeze(0),
                ...
                if args.attack_method == 'direct':
                    ...
                    if args.algo in ('FNI', 'DARRL'):
                        ...
                        print("DEBUG action before attack: ", actions, " adv action: ", adv_actions, "action after attack: ", adv_action_fromState)
